...
class ExecuteProcedureView(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "procedure_name": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Procedure name"
                ),
                "output_parameters": openapi.Schema(
                    type=openapi.TYPE_OBJECT,
                    properties={
                        "return_value": openapi.Schema(
                            type=openapi.TYPE_INTEGER, description="Return value"
                        )
                    },
                ...
                "parameters": openapi.Schema(
                    type=openapi.TYPE_OBJECT,
                    properties={
                        "mali_year": openapi.Schema(
                            type=openapi.TYPE_INTEGER, description="Mali Year code"
                        ),
                        "place_gcode": openapi.Schema(
                            type=openapi.TYPE_INTEGER, description="Places Gcode"
                        ),
                        "company_code": openapi.Schema(
                            type=openapi.TYPE_STRING, description="Company Codes"
                        ),
                        "action": openapi.Schema(
                            type=openapi.TYPE_STRING, description="Action"
                        ),
                    },
                    required=["MaliYearGcode", "PlacesGcode", "company_codes"],
                ),
                "form_id": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Form ID"
                ),
            },
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if procedure.is_public:
                ...
                if not form.procedures.filter(id=procedure.id).exists():
                    return Response(
                        {"error": "Procedure is not associated with the given form"},
                        status=status.HTTP_403_FORBIDDEN,
                    )
        except Procedure.DoesNotExist:
            return Response(
                {"error": "Procedure not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if procedure.is_public:
            ...
            if mali_year:
                if not MainAccess.objects.filter(
                    user=user, mali_year__name__icontains=mali_year
                ).exists():
                    flag = False

            if place_gcode:
                if not MainAccess.objects.filter(
                    user=user, place_gcode__name__icontains=place_gcode
                ).exists():
                    flag = False

            if company_code:
                if not MainAccess.objects.filter(
                    user=user, company_code__name__icontains=company_code
                ).exists():
                    flag = False
            ...
            if parameters_action:
                ...
                filter_kwargs = {"user": user, "form_id": form_id, action_key: True}
            ...
            action_key_query_params = actions_mapping.get(int(action_query_param))
            filter_kwargs = {"procedure": procedure, action_key_query_params: True}
        ...
        if not procedure_name:
            return Response(
                {
                    "error": "Procedure name, parameters, and output parameters are required in the body"
                },
